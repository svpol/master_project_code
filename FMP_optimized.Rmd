...
This function will be used to collapse gene names into one row: e.g. GENE1, GENE1.1, GENE1.2, GENE1.1.2.1 will be collapsed to GENE1 and the median value will be taken for each cell.

```{r collapse_gene_names}
...
For each Seurat object, filtering out cells with < 100 and > 2500 genes (> 7000 genes for GSE148673) and with > 10% of mitochondrial content:
...
```{r stacked_cluster_dataset_merged}
...
```{r stacked_cluster_dataset_RPCA}
...
```{r stacked_cluster_dataset_harmony}
